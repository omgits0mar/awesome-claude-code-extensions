...
| Kind | Meaning |
| --- | --- |
| `plugin` | An installable Claude Code bundle that can include several component types. |
...
| `hook` | Automation triggered by Claude Code lifecycle events. |
...
| `interface` | A GUI, IDE, mobile, remote, or terminal interface for Claude Code. |
...
Cross-agent projects are included only when Claude Code compatibility is explicit or follows directly from a supported open standard.
